...
df.columns.values
df = df.drop(['pixel0', 'pixel1', 'pixel2', 'pixel3', 'pixel4', 'pixel5',
       ...
       'pixel782', 'pixel783'],axis = 1)

df['ImageId'] = np.arange(1,28001)
df['Label'] = y_pred


df.to_csv('Senddds16.csv',index =False)

#dd =  pd.read_csv('Senddds16.csv')

#test_loss, test_accuracy = model.evaluate(x2,y_pred)
